...
getAttachElement:function(attachModel,opts){
	...
	if((priv & 1)==1){//阅读权限
		var documentType = "";
	   	...
	   	var presentation = "fodp, odp, otp, pot, potm, potx, pps, ppsm, ppsx, ppt, pptm, pptx";
	   	if(text.indexOf(ext) > -1){
	   		documentType = "text";
	   	}
	   	if(spreadsheet.indexOf(ext) > -1){
	   		documentType = "spreadsheet";
	   	}
	   	if(presentation.indexOf(ext) > -1){
	   		documentType = "presentation";
	   	}
	   	if(documentType != ""){
	   		menuData.push({name:"查看",action:function(sid,ext,fileName){
				openOnlyOffice(sid,ext,fileName);
			} ,extData:[attachModel.sid,attachModel.ext,attachModel.fileName]});
	   	}else if(this.isImage(ext)){//如果是图片
			...
			//判断是否是doc  docx   xls   xlsx
//			if(ext=="doc" || ext=="xls" || ext=="docx" || ext=="xlsx"){
//				menuData.push({name:"快速预览",action:function(sid,fileName){
//					var url =contextPath+"/system/core/ntko/officePreview.jsp?attachId="+sid;
//					openFullWindow(encodeURI(url),"快速预览");
//				} ,extData:[attachModel.sid,attachModel.fileName]});
//			}
		...
		}else if(this.isText(ext)){//文本类型
			menuData.push({name:"查看",action:function(sid,fileName){
				var textUrl = contextPath +  "/attachmentController/textPreview.action?attachmentId="+sid+"&attachmentName="+fileName+"&ext="+ext;
				openFullWindow(encodeURI(textUrl),"文本预览");
			} ,extData:[attachModel.sid,attachModel.fileName]});
		}else if(this.isAip(ext)){//aip文件
			menuData.push({name:"预览",action:function(sid,fileName){
				var ntkoURL = contextPath +  "/system/core/aip/aipView.jsp?attId="+sid;
				openFullWindow(encodeURI(ntkoURL),"版式文件预览");
			} ,extData:[attachModel.sid,attachModel.fileName]});
		}
	}
...
getAttachElementXzfy:function(attachModel,opts){
	var opts = opts;
	if(!opts){
		opts = {};
	}
	...
	var ext = attachModel.ext;
    if(!ext){
        var filename = attachModel.fileName;
        var index1=filename.lastIndexOf(".");
        var index2=filename.length;
        ext=filename.substring(index1,index2);
    }
    var fileDivId = '';
    if(opts.params != undefined){
        fileDivId = opts.params.fileDivId;
    }
	var render = "<div style=\"font-size:12px\" ";
	if(fileDivId != undefined && fileDivId != ''){
	    render += " id=\""+fileDivId+"\">";
	}else{
	    render += ">";
	}
	...
	if((priv & 1)==1){//阅读权限
		if(this.isImage(ext)){//如果是图片
			...
			menuData.push({name:"预览",action:function(sid){
				...
				top.$.picExplore({src:contextPath+"/attachmentController/downFile.action?id="+sid});
				openFullWindow(url,"在线预览");
				//openOnlyOffice(sid,ext,attachModel.fileName);
			} ,extData:[attachModel.sid]});
		}else if(this.isOffice(ext)){//如果是office文档
			menuData.push({name:"查看",action:function(sid,fileName){
				var ntkoURL = contextPath +  "/system/core/ntko/indexNtko.jsp?attachmentId="+sid+"&attachmentName="+fileName+"&moudle=workFlow&op=7";
...
//			}
		...
		}else if(this.isAip(ext)){//aip文件或者ofd文件
			menuData.push({name:"预览",action:function(sid,fileName){
				var ntkoURL = contextPath +  "/system/core/aip/aipView.jsp?attId="+sid;
				openFullWindow(encodeURI(ntkoURL),"版式文件预览");
			} ,extData:[attachModel.sid,attachModel.fileName]});
...
isText:function(ext){//文本类型
	ext = ext.toLowerCase();
	if(ext=="txt" || ext=="html" || ext=="htm" || ext=="jsp"||ext=="php"||ext=="aspx"||ext=="asp"){
		return true;
	}
	return false;
},
